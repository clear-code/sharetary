...
function createFilter(requestQueries) {
  ...
  if (requestQueries.authors) {
    var targetAuthors = requestQueries.authors.trim().split(/[\|,\s]+/).map(function(author) {
      return author.trim();
    ...
    if (targetAuthors.length > 0) {
      filters.push(
        '( ' +
        targetAuthors.map(function(author) {
  ...
  if (requestQueries.repository)
    filters.push('repository == "' + requestQueries.repository + '"');
...
function inheritedFilters(requestQueries) {
  return {
    authors:    requestQueries.authors,
    repository: requestQueries.repository,
...
var keywordSearchMatchColumns = 'type,title,title,description,author,uri,repository';
