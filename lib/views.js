...
application.get('/filters-actors-and-tags', function(request, response) {
  getAllActorsAndTags().then(function(result) {
    response.partial('partials/filters-actors-and-tags', {
...
application.get('/archive.csv', function(request, response) {
  var columns = request.query.columns || 'type,actor,uri,tags';
  columns = columns.trim().split(/\s*,\s*/);
  getArchiveResult(request).then(function(result) {
    response.csv(result.records.map(function(record) {
